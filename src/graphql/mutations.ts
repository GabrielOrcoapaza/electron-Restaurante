...
export const TRANSFER_ITEMS = gql`
    mutation TransferItems(
        $fromOperationId: ID!
        $toTableId: ID!
        $detailIds: [ID!]!
        $branchId: ID!
        $createNewOperation: Boolean
    ) {
        transferItems(
...
export const ASSIGN_CATEGORY_TO_PRINTER = gql`
    mutation AssignCategoryToPrinter(
        $printerId: ID!
        $categoryId: ID!
        $priority: Int
    ) {
