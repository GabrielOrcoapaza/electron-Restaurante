  ...
  const handleExportExcel = async () => {
    ...
    try {
      if (viewMode === 'orders') {
        ...
        const result = await downloadEmployeeOrdersReport(operations, employeeName, range);
        showToast(result.message || 'Reporte descargado en Excel.', 'success');
      } else {
        if (!dishLines.length) {
          showToast('No hay platos para exportar en este periodo.', 'warning');
          return;
        }
        const result = await downloadEmployeeDishesReport(dishLines, employeeName, range);
        showToast(result.message || 'Reporte descargado en Excel.', 'success');
      }
    } catch (error) {
